...
with open("tests/twenty_newsgroups/data/clean_data/word_to_idx.pickle", "rb") as w2i_in:
    word_to_idx = pickle.load(w2i_in)

#Load Initial Embedding matrix
embed_matrix = np.load("tests/twenty_newsgroups/data/clean_data/embedding_matrix.npy")
...
npz = np.load(open("logdir_190402_1738/model_params.npz", 'rb'), allow_pickle=True)
...
glove_embeddings = dict(get_coefs(*o.split(" ")) for o in open("tests/twenty_newsgroups/glove_embeddings/glove.6B.300d.txt"))
